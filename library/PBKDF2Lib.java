...
public class PBKDF2Lib {
    ...
    public String createPassword(String plainPassword) {
        byte someBytes[] = new byte[PBKDF2Lib.SALT_LENGTH];
        ...
        try {
            ...
            Logger.getLogger(PBKDF2Lib.class.getName()).log(Level.SEVERE, null, ex);
        }
        KeySpec ks = new PBEKeySpec(plainPassword.toCharArray(), encodedSalt.getBytes(), PBKDF2Lib.ITERATIONS, PBKDF2Lib.KEY_LENGTH);
        SecretKey s;
        try {
            s = f.generateSecret(ks);
            String encodedKey = Base64.getEncoder().encodeToString(s.getEncoded());
            String hashedKey = "PBKDF2$sha256$" + PBKDF2Lib.ITERATIONS + "$" + encodedSalt + "$" + encodedKey;
            return hashedKey;
